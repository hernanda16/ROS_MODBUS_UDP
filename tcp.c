...
int main(int argc, char **argv)
{
    ...
    if (argc < 6)
    {
        printf("usage: test2 ip_adrs port unit reg_no num_regs\n"
               "eg test2 169.254.196.196 1296 5 0 10\n");
        return 1;
    }
    ...
    // printf("ip_adrs = %s port = %d unit = %d reg_no = %d num_regs = %d\n", port,
    ...
    // server.sin_port = htons(9696); /* ASA standard port */
    ...
    obuf[5] = 6;
    obuf[6] = unit;
    obuf[7] = 3; // 3 untuk read, 6 untuk write
    obuf[8] = reg_no >> 8;
    obuf[9] = reg_no & 0xff;
    obuf[10] = num_regs >> 8;
